...
export default function Dashboard() {
  ...
  // Helpers de atuadores (controle manual ON/OFF)
  const actuatorTipo = (comp: CompCfg) => {
    const m = (comp.model || comp.label || comp.type || '').toUpperCase()
    if (m.includes('RELE') || m.includes('RELAY')) return 'rele'
    if (m.includes('LED')) return 'led'
    if (m.includes('VIB') || m.includes('MOTOR')) return 'vibracao'
    return m.toLowerCase()
  }
  const sendActuator = async (comp: CompCfg, command: 'ON' | 'OFF') => {
    try {
      const tipo = actuatorTipo(comp)
      const r = await fetch(`${API_URL}/api/actuator`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tipo, pin: comp.pin, command })
      })
      if (!r.ok) throw new Error(`falha (${r.status})`)
      setAlerts(old => [{ ts: Date.now(), kind: 'normal' as const, text: `atuador ${tipo} pino ${comp.pin} -> ${command}` }, ...old].slice(0,30))
    } catch (e: any) {
      setAlerts(old => [{ ts: Date.now(), kind: 'critico' as const, text: `erro atuador pino ${comp.pin}: ${e.message}` }, ...old].slice(0,30))
    }
  }
      ...
      for (const c of comp) {
        ...
        if (found.kind === "object" && found.value && typeof found.value === "object") {
          for (const [subk, subv] of Object.entries(found.value)) {
            ...
            byKey[subName] = {
              raw: subv,
              num: Number.isFinite(num) ? num : NaN,
              unidade: c.unit || "",
        ...
        {device?.components?.length ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {device.components.map((c) => {
              ...
              const isActuator = c.type === 'atuador' || /rele|relay|led|vib|motor/i.test(c.model || c.label || c.type || '')
              return (
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {isActuator && (
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => sendActuator(c, 'ON')}>Ligar</Button>
                        <Button size="sm" variant="outline" onClick={() => sendActuator(c, 'OFF')}>Desligar</Button>
                      </div>
                    )}
                    {/* submetricas */}
